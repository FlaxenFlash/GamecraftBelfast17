...
public class MinMaxRangeDrawer : PropertyDrawer
{
	...
	public override void OnGUI( Rect position, SerializedProperty property, GUIContent label )
	{
		...
		else
		{
			...
			EditorGUI.LabelField( new Rect( position.x + position.width - 24f, position.y + yDivision, position.width, yDivision ), range.maxLimit.ToString( "0.##" ) );
			EditorGUI.MinMaxSlider( new Rect( position.x + 24f, position.y + yDivision, position.width - 54f, yDivision ), ref newMin, ref newMax, range.minLimit, range.maxLimit );
			
			EditorGUI.LabelField( new Rect( position.x + xDivision, position.y, xDivision, yDivision ), "From " );
			newMin = Mathf.Clamp( EditorGUI.FloatField( new Rect( position.x + xDivision + 35, position.y, xDivision - 35, yDivision ), newMin ), range.minLimit, newMax );
